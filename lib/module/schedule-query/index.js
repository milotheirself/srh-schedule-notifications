    ...
    page.on('load', () => {
      ...
      setTimeout(async () => {
        ...
        if (page.url().toLowerCase().indexOf('login') != -1) {
          await page.type('input[name="txtUser"]', `${external.options.signin.name}`);
          await page.type('input[name="txtPass"]', `${external.options.signin.pass}`);
...
internal.fetchTable = async ({ page, urn, wee, tas }) => {
  ...
  hashSum.update(await nodeFiles.readFile(`./result/image/${wee}-${tas}.png`, { encoding: 'base64' }));
